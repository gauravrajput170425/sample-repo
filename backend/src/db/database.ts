...
// Define permission type for shared lists
export type SharePermission = 'read' | 'write';

// Define shared list entry
...
class Database {
  ...
  private todos: Map<string, Todo[]> = new Map(); // Map of listId to todos
  private userTodoLists: Map<string, string[]> = new Map(); // Map of userId to listIds
  private sharedTodoLists: Map<string, SharedListEntry[]> = new Map(); // Map of userId to shared lists
  ...
  public getUserByUsernameOrEmail(usernameOrEmail: string): User | undefined {
  ...
  public getUserTodoLists(userId: string): TodoList[] {
  ...
  public getSharedTodoLists(userId: string): TodoList[] {
  ...
  public getAllAccessibleLists(userId: string): TodoList[] {
  ...
  public getTodoListById(listId: string): TodoList | undefined {
  ...
  public isListOwnedByUser(userId: string, listId: string): boolean {
    const userListIds = this.userTodoLists.get(userId) || [];
    return userListIds.includes(listId);
  }

  // Check if a todo list is shared with a user
  public isListSharedWithUser(userId: string, listId: string): SharedListEntry | undefined {
    const sharedEntries = this.sharedTodoLists.get(userId) || [];
  ...
  public hasAccessToList(userId: string, listId: string): boolean {
  ...
  public hasWriteAccessToList(userId: string, listId: string): boolean {
  ...
  public getListOwner(listId: string): string | undefined {
  ...
  public addTodoList(todoList: TodoList, userId: string): TodoList {
  ...
  public removeSharing(listId: string, ownerUserId: string, targetUserId: string): boolean {
    // Check if the list exists and the owner owns it
    if (!this.isListOwnedByUser(ownerUserId, listId)) {
      return false;
    }
  ...
  public updateTodoList(listId: string, updates: Partial<TodoList>): TodoList | null {
  ...
  public deleteTodoList(listId: string, userId: string): boolean {
  ...
  // Todo methods
  public getTodos(listId: string): Todo[] {
    return this.todos.get(listId) || [];
  }

  public getTodoById(listId: string, todoId: string): Todo | undefined {
    const todos = this.todos.get(listId);
    if (!todos) return undefined;
    return todos.find(todo => todo.id === todoId);
  }

  public addTodo(listId: string, todo: Todo): Todo | null {
  ...
  public updateTodo(listId: string, todoId: string, updates: Partial<Todo>): Todo | null {
    const todos = this.todos.get(listId);
  ...
  public deleteTodo(listId: string, todoId: string): boolean {
    const todos = this.todos.get(listId);
